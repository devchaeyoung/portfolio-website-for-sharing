...
> ### 💡문제 1. form data 에 들어있는 값으로만 update가 안되던 문제
...
#### 💡 문제 2. 두 개의 식별자로 인한 혼란

스켈레톤 코드로 이미 구현되어있던 유저컬렉션 id key의 uuid4 와, 몽구스객체가 생성될 때 자동으로 부여되는 ObjectId, 두 개의 식별자를 사용함으로 인한 프론트와 서버간의 혼란 야기
...
> ### 💡 문제 3. POST, PUT,DELETE 요청에, 데이터베이스에 저장되기 전 응답하던 문제
>
...
> ### 💡 문제 4. static async findByEmail({email})함수에서 검색이 올바르게 이루어지지 않던 문제
...
> ### 💡 문제 5. Error: Cannot set headers after they are sent to the client 오류 문제
...
> ### 💡 문제 6. 특정 주소에서 리로드 시 로그인창이 뜨는 문제 !
...
- 이미지 업로드 후 리로드를 해야 클라이언트에 적용이 됨
...
발급받은 token을 이용해서 서버로 get요청을 보내 유저에 대한 상세정보를 받아와 dispatch payload에 응답 데이터를 넣고 유저 상세정보와 이미지를 최신화 했습니다.
